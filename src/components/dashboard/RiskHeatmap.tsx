    ...
    <div className="bg-card border border-border rounded-lg p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Risk severity trends by category over time</p>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {heatmapData.map(row => (
              <tr key={row.category}>
                <td className="text-xs font-medium text-foreground py-1 pr-3">{row.category}</td>
                {periodKeys.map(key => (
