    ...
    <div className="bg-card border border-border rounded-lg p-5">
      ...
      <div className="relative">
        <ResponsiveContainer width="100%" height={180}>
          <PieChart>
            <Pie data={data} cx="50%" cy="50%" innerRadius={55} outerRadius={80} paddingAngle={3} dataKey="value" strokeWidth={0} onClick={handleClick} className="cursor-pointer">
      ...
      <div className="grid grid-cols-2 gap-2 mt-2">
        {data.map(d => (
          <button key={d.name} onClick={() => navigate({ to: '/controls', search: { status: d.filter } })} className="flex items-center gap-2 text-xs hover:bg-accent/50 rounded px-1 py-0.5 transition-colors cursor-pointer">
