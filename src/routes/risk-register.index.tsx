...
const statusStyles: Record<string, string> = {
  ...
  mitigated: 'bg-status-passing/15 text-status-passing',
  accepted: 'bg-muted text-muted-foreground',
  transferred: 'bg-status-in-progress/15 text-status-in-progress',
...
const riskFields: FieldDef[] = [
  ...
  { name: 'impact', label: 'Impact (1-5)', type: 'number', required: true, min: 1, max: 5 },
  {
    ...
    options: [
      ...
      { value: 'accepted', label: 'Accepted' }, { value: 'transferred', label: 'Transferred' },
...
function RiskRegisterPage() {
  ...
  const { data: risks, loading, insert, update, remove } = useSupabaseCrud('risks');
  ...
  const matrix = useMemo(() => {
    ...
    for (let l = 1; l <= 5; l++) {
      for (let i = 1; i <= 5; i++) {
        m[`${l}-${i}`] = risks.filter(r => r.likelihood === l && r.impact === i);
      }
    }
    return m;
  }, [risks]);
  ...
  return (
    <div className="space-y-6 animate-slide-in">
                  ...
                  [1, 2, 3, 4, 5].map(impact => {
                    ...
                    return (
                      <div key={`${likelihood}-${impact}`}
                        ...
                        {cellRisks.length > 0 && (
                          <span className={`text-xs font-bold px-2 py-0.5 rounded ${scoreColor(score)}`}>{cellRisks.length}</span>
                        )}
                      </div>
                    );
                  })
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {[...risks].sort((a, b) => (b.risk_score ?? 0) - (a.risk_score ?? 0)).map(r => (
              <tr key={r.id} className="border-b border-border hover:bg-muted/50 transition-colors cursor-pointer"
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs font-bold px-2 py-0.5 rounded ${scoreColor(r.risk_score)}`}>{r.risk_score ?? '—'}</span>
                </td>
                ...
                <td className="px-4 py-3">
                  <span className={`text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded ${statusStyles[r.status] ?? 'bg-muted text-muted-foreground'}`}>{r.status}</span>
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                    <button onClick={() => { setEditing({ title: r.title, description: r.description, category: r.category, likelihood: r.likelihood, impact: r.impact, status: r.status, mitigation_plan: r.mitigation_plan, _id: r.id }); setFormOpen(true); }}
                    ...
                    <button onClick={() => setDeleteTarget({ id: r.id, title: r.title })}
        ...
        {risks.length === 0 && (
          <div className="text-center py-12 text-muted-foreground text-sm">No risks recorded yet.</div>
        )}
      </div>

      <EntityFormDialog open={formOpen} onOpenChange={setFormOpen}
        title={editing ? 'Edit Risk' : 'Add Risk'} fields={riskFields}
