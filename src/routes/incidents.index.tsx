...
import { Search, Loader2, Plus, Pencil, Trash2, Download, Flame, Filter, AlertCircle } from 'lucide-react';
...
function IncidentsPage() {
  ...
  const { data: incidents, loading, error, refetch, insert, update, remove, bulkRemove, bulkUpdate } = useSupabaseCrud('incidents');
    ...
    <div className="space-y-5 animate-fade-up">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{incidents.length} incidents tracked{loading && <span className="inline-flex items-center gap-1 ml-2 text-xs"><Loader2 className="h-3 w-3 animate-spin" />refreshing</span>}</p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" aria-label="Search incidents" placeholder="Search incidents..." value={search} onChange={e => updateSearch({ q: e.target.value })}
