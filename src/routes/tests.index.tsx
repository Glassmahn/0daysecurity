...
import { Search, Loader2, Plus, Pencil, Trash2, CheckCircle, XCircle, Clock, AlertTriangle } from 'lucide-react';
...
export const Route = createFileRoute('/tests/')({
  ...
  head: () => ({
    meta: [
      { title: 'Tests — WatchDog Security' },
      { name: 'description', content: 'Compliance test management' },
    ],
  }),
});

const statusStyles: Record<string, string> = {
  passing: 'bg-status-passing/15 text-status-passing',
  failing: 'bg-status-failing/15 text-status-failing',
  pending: 'bg-status-in-progress/15 text-status-in-progress',
  error: 'bg-severity-high/15 text-severity-high',
...
const testFields: FieldDef[] = [
  ...
  { name: 'description', label: 'Description', type: 'textarea', placeholder: 'Describe this test...', max: 2000 },
  {
    ...
    options: [
      { value: 'passing', label: 'Passing' }, { value: 'failing', label: 'Failing' },
...
function TestsIndexPage() {
  ...
  const { data: tests, loading, insert, update, remove } = useSupabaseCrud('tests');
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          <div className="text-2xl font-bold text-status-passing">{stats.passing}</div>
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          <div className="text-2xl font-bold text-status-in-progress">{stats.pending}</div>
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map(t => (
              <tr key={t.id} className="border-b border-border hover:bg-muted/50 transition-colors cursor-pointer"
                ...
                <td className="px-4 py-3 text-muted-foreground text-xs hidden lg:table-cell">
                  {t.last_run ? new Date(t.last_run).toLocaleDateString() : '—'}
                </td>
                <td className="px-4 py-3">
                  <span className={`text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded ${statusStyles[t.status] ?? 'bg-muted text-muted-foreground'}`}>
                    {t.status}
                  </span>
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                    <button onClick={() => { setEditing({ name: t.name, description: t.description, status: t.status, result: t.result, schedule: t.schedule, _id: t.id }); setFormOpen(true); }}
                    ...
                    <button onClick={() => setDeleteTarget({ id: t.id, title: t.name })}
      ...
      <EntityFormDialog open={formOpen} onOpenChange={setFormOpen}
        title={editing ? 'Edit Test' : 'New Test'} fields={testFields}
