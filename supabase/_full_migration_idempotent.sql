-- Drop everything first in case of partial state
DROP TABLE IF EXISTS public.evidence_recollection_attempts CASCADE;
...
DROP TABLE IF EXISTS public.risk_treatment_approvals CASCADE;
DROP TABLE IF EXISTS public.training_quiz_attempts CASCADE;
DROP TABLE IF EXISTS public.training_quiz_questions CASCADE;
DROP TABLE IF EXISTS public.vendor_portal_submissions CASCADE;
DROP TABLE IF EXISTS public.vendor_portal_tokens CASCADE;
...
DROP TABLE IF EXISTS public.audit_evidence_requests CASCADE;
...
DROP TABLE IF EXISTS public.access_review_assignments CASCADE;
DROP TABLE IF EXISTS public.access_review_campaigns CASCADE;
DROP TABLE IF EXISTS public.vendor_assessments CASCADE;
DROP TABLE IF EXISTS public.policy_acknowledgments CASCADE;
DROP TABLE IF EXISTS public.trust_portal_shares CASCADE;
...
CREATE TABLE public.controls (
  ...
  title TEXT NOT NULL,
  description TEXT,
  status TEXT NOT NULL DEFAULT 'not_started' CHECK (status IN ('implemented','partially_implemented','not_implemented','not_applicable','not_started','failing','in_progress')),
  category TEXT,
...
CREATE TABLE public.evidence (
  ...
  status TEXT NOT NULL DEFAULT 'valid' CHECK (status IN ('valid','expired','pending_review','rejected','needs_recollection')),
...
-- personnel: team members tracked for access reviews and training.
...
CREATE POLICY "All authenticated users view personnel"
...
DROP POLICY IF EXISTS "Public can view vendor tokens by token" ON public.vendor_portal_tokens;
CREATE POLICY "Public can view vendor tokens by token"
  ON public.vendor_portal_tokens FOR SELECT TO anon
  USING (true);
DROP POLICY IF EXISTS "Admins manage vendor tokens" ON public.vendor_portal_tokens;
...
DROP POLICY IF EXISTS "Public can insert submissions via portal" ON public.vendor_portal_submissions;
CREATE POLICY "Public can insert submissions via portal"
  ON public.vendor_portal_submissions FOR INSERT TO anon
  WITH CHECK (true);
DROP POLICY IF EXISTS "Admins can view submissions" ON public.vendor_portal_submissions;
...
DROP POLICY IF EXISTS "Users can view own quiz attempts" ON public.training_quiz_attempts;
...
DROP POLICY IF EXISTS "Users can insert own quiz attempts" ON public.training_quiz_attempts;
...
DROP POLICY IF EXISTS "Authenticated users can view treatment approvals" ON public.risk_treatment_approvals;
CREATE POLICY "Authenticated users can view treatment approvals"
  ...
  USING (true);
DROP POLICY IF EXISTS "Admins/analysts manage treatment approvals" ON public.risk_treatment_approvals;
...
DROP POLICY IF EXISTS "Users can view webhook endpoints" ON public.webhook_endpoints;
CREATE POLICY "Users can view webhook endpoints"
  ON public.webhook_endpoints FOR SELECT TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Admins/analysts manage webhook endpoints" ON public.webhook_endpoints;
...
DROP POLICY IF EXISTS "Users can view webhook deliveries" ON public.webhook_deliveries;
...
DROP POLICY IF EXISTS "Users can view recollection attempts" ON public.evidence_recollection_attempts;
CREATE POLICY "Users can view recollection attempts"
  ON public.evidence_recollection_attempts FOR SELECT TO authenticated
  USING (true);

DROP POLICY IF EXISTS "System can insert recollection attempts" ON public.evidence_recollection_attempts;
